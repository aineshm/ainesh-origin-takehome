 ...
 *
 * The model's arguments arrive pseudonymized; they are de-anonymized via the
 * vault into real values BEFORE the real tool runs (so the tool — and the audit
 * trace it writes — sees the true values). The tool's rich `data` is then
 * re-anonymized before being returned to the model as `content`, so the model
 * never sees raw PHI. For `create_task` the real `task_id` is surfaced as
 * `taskId`. Throws on an unknown tool name. Must be called inside the caller's
 * `withItemContext`.
 */
export async function executeTool(
  ...
  const input = vault.deAnonymize(rawInput);
  const reanonymize = (data: unknown): string =>
    vault.reAnonymizeText(JSON.stringify(data));
  ...
  switch (name) {
    case "search_patient": {
      ...
      return { content: reanonymize(result.data) };
    }
    ...
    case "create_task": {
      ...
      return {
        content: reanonymize(result.data),
